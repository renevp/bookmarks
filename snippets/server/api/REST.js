...
.put(function(){
    writeHeaders(this);
    // check to see if changes parameters were specified
    var record = this.request.body.update;
    if(!record){
        this.response.end('Notihing requested...\n');
        return;
    }
    
    var update = Snippets.upsert({_id: record.id},{$set: record.changes});
    console.log(update);
    this.response.end('Snippet updated! \n');
})
.delete(function(){
    writeHeaders(this);
    // check if an ID was sent for deletion...
...
function writeHeaders(self){
    ...
    self.response.setHeader("Access-Control-Allow-Header", "Origin, X-Requested-With, Content-Type, Accept");
}